...
Electricity prices are referenced from the industrial electricity price forecast for the Mountain region (includes New Mexico) provided by the EIA in their \acrlong{steo} (\acrshort{steo}) projections out to 2023 \citep{eia_short-term_2021}. While industrial pricing differs slightly from wholesale, it more closely mimics wholesale prices than residential or commercial rates and was therefore selected as a wholesale proxy for the cost model. The Forecast Tool in Excel projected prices out to 2050 (Figure \ref{fig:electricity_pricing}). This tool uses the Exponential Triple Smoothing algorithm with seasonality patterns to forecast time series data \citep{microsoft_forecastets_2021}. For the deterministic cost model, electricity prices are directly sampled from the forecast and multiplied by the PPA \textit{Contract rate above wholesale} value listed in Table \ref{tab:cm_econ_params} for any year when capacity increases. This simulates amending the PPA with a local utility whenever new capacity is available for power sales. Electricity pricing is held flat compared to the previous year when no capacity change occurs.

\begin{figure}[!htp]
\centering
\includegraphics[width=\textwidth]{templates/images/Figure-EIA_Electricity_Forecast.png}
\caption[Electricity price forecast]{Price of electricity forecast based on EIA Short-Term Energy Outlook \protect\citep{eia_short-term_2021}. A seasonal signal is visible due to the quarterly sampling of the data.}
