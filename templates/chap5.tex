...
\subsubsection{Hyperparameter Tuning}
Multiple hyperparameters determine the performance of a decision tree classifier. Here we consider six parameters, each of which are tuned using the Stratified k-Fold CV method described in Section \ref{ch5:strat_kfold_cv} with 10 folds and the multi-class one-vs-rest ROC AUC as the scoring metric. The examples provided follow the tuning procedure for WDS4.

First, \textbf{max_depth} and \textbf{criterion} were tuned together. max\_depth limits tree expansion by capping the number of number of parameter evaluations (tree nodes) considered before a classification label assignment. Criterion refers to the quality metrics defined above. Figure XX illustrates the relative insensitivity the classifier has to the selected criterion, while tree depth plays a stronger role in performance. A maximum AUC score is observed with a max\_depth of 8 and criterion choice of “Entropy.”
