...
%\begin{figure}[!htp]
%\centering
%\includegraphics[width=\textwidth]{templates/images/Figure-Structural_Entropy_Combined_Maps.png}
%\caption[Structural uncertainty map]{Structural uncertainty measured using Shannon entropy. A. Normalized entropy values calculated from the four predictive models shown in Figure \ref{fig:combined_maps}. B. Probability-averaged prediction map with uncertainty masking. Normalized entropy values (>0.7) are blacked out, all others set alphas on a gray layer mask.}
%\label{fig:structural_entropy_map}
%\end{figure}
...
Single BNN passes can be evaluated the same way as the deterministic NN using a confusion matrix and ROC curves (Figures \ref{fig:bnn_confusion_matrix}, \ref{fig:bnn_auc}).
