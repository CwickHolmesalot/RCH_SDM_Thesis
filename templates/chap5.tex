...
\label{eq:logreg_form}
    \hat{y} = {\bf{w^Tx}} = w_0 + w_1 * x_1 + w_2 * x_2 \ldots + w_n * x_n 
\end{equation}
...
\caption[XGBoost AUC curves]{ROC AUC curves for tuned XGB model trained on WDS4.}
\label{fig:xgb_auc}
\end{figure}
...
The concept of \acrlong{ann}s (\acrshort{ann}s or just \acrshort{nn}s) comes from a simplified representation of neuron activity in the brain \citep[p.\ 394]{hastie_elements_2009}. Multiple inputs are fed into a neuron cell from many branches on one end, and given the right combination of those inputs, the cell will fire and propagate a signal to the next connected neuron.

In neural networks, cells are represented by logistic units (Figure \ref{fig:nn_schematic}). Each unit acts like a logistic regression operation, where inputs are scaled by weights, and the linear sum of the weighted inputs are passed through an “activation function” to determine if the output is a 0 or a 1. As in logistic regression, the sigmoid function (Equation \ref{eq:sigmoid}) is a common choice, but many other activation functions like the \acrlong{relu} (\acrshort{relu}) are used as well \citep{brownlee_gentle_2019}.

\begin{figure}[!htp]
\centering
\includegraphics[width=\textwidth]{templates/images/Figure-ANN.png}
\caption[Neural network schematic]{Schematic of a simplified logistic unit (Left) and a fully-connected artificial neural network (Right) with a single hidden layer made up of logistic units $a_1$ through $a_m$. Bias units (subscript 0) traditionally have a value of 1, but are weighted in linear combinations like other units. Four units in the output layer make this a four-class classifier.}
...
where $g$ is the activation function and $\bf{W}$ are the weights. For the first hidden layer, $\bf{X}$ will be the predictor values from the input layer as shown in Figure \ref{fig:nn_schematic}. For any additional hidden layers, $\bf{X}$ consists of the outputs of units from the preceding layer. Here, $\bf{z}$ is described in matrix notation but has the same form as Equation \ref{eq:logreg_form}. 
...
\begin{minipage}{.36\linewidth}
    ...
    \caption[Neural network structural flow]{Flow chart for the neural network, constructed using TensorFlow \protect\citep{abadi_tensorflow_2016}.}
    \label{fig:ann_text_structure}
\end{minipage}
\hfill
\begin{minipage}{.60\linewidth}
    ...
    \label{fig:ann_dot_structure}
\end{minipage}
\end{figure}
This analysis uses a neural network constructed with TensorFlow \citep{abadi_tensorflow_2016}. The network architecture includes hidden layers of the same size as the input layer. This results in a 4-layer structure comprising an input layer of size (1x24), two hidden layers also (1x24), and an output layer of (1x4), assuming all data set features are used (Figures \ref{fig:ann_text_structure}, \ref{fig:ann_dot_structure}). In addition, dropout is included after each of the hidden layers. Dropout helps prevent overfitting by randomly assigning inputs to zero, temporarily severing a defined percentage of node connections during each step of the training process.
...
Likewise, the choice of activation functions for each layer may also impact the performance of the network. ReLU functions overcome several limitations observed with other activation choices like the sigmoid function, e.g., limited sensitivity and vanishing gradients, making ReLU the current preferred default choice \citep{brownlee_gentle_2019}. Both hidden layers in the network described here apply ReLU activation functions.

The Adam optimization method was chosen for training the network due to its computational efficiency and present-day status as the recommended general-purpose optimizer \citep{brownlee_gentle_2017}. A description of Adam is beyond the scope of this thesis, but more information is available in the literature \citep{kingma_adam_2017}.

The training process updates the network weights using gradients calculated from the training data in each training epoch \citep[p.\ 397]{hastie_elements_2009}. Successive epochs train the NN to match the training data even more, so evaluating loss, AUC, or other metrics on both the training and validation sets is important for tuning the number of epochs to avoid overfitting. Training can also proceed with mini-batches, that is, small subsets of the training set, rather than the whole set at once. This method makes the training results noisier, but generally speeds up learning while adding a regularization effect to changes in the weights \citep{brownlee_how_2019}.  

Parameter tuning therefore focuses on five hyperparameters:
\begin{itemize}[itemsep=2pt]
    \item \textbf{Learning rate}: scales the size of the updates to weights during each step of the training process. The Adam optimizer adjusts this rate during training, so the value here defines an initial learning rate. Initially set to 0.001.
    \item \textbf{Lambda}: the L2 weight regularization term, i.e., the $\lambda$ in equation \ref{eq:nn_cost_function}. Initially set to $1\times10^{-4}$.
...
\caption[Neural network hyperparameter tuning]{Neural network hyperparameter tuning results for A. learning rate, B. lambda ($\lambda$), C. batch size, D. drop rate, using WDS4. The orange lines track the AUC after 100 training epochs, averaged over 10-folds of stratified cross-validation for each parameter value. The blue lines show average loss. Red dashed lines indicate values selected for the final model.}
\label{fig:nn_tuning_plots}
\end{figure}

The stratified k-fold cross-validation strategy described in Section \ref{ch5:strat_kfold_cv} was used for tuning the neural network hyperparameters. Training continued for 100 epochs, and since TensorFlow tracks metric values for both the training and validation sets at each epoch, the two sets were kept separate during the 10-fold CV process. In the figures and discussion that follow, results are described for WDS4. Full results for all three data sets are provided in Table \ref{tab:nn_tuning}. 
...
\end{tabular}
\singlespacing
\caption[Neural network hyperparameter tuning results]{Neural network hyperparameter tuning results for each data set. Accuracy and AUC metrics are split into train (in-sample) and test (out-of-sample) values.}
\label{tab:nn_tuning}
\end{wraptable}
...
\caption[Neural network training loss]{Neural network training results as a function of epoch count for WDS4. Calculated loss (Left) and AUC (Right) for the training (blue) and validation (green) data subsets match and begin to separate near the 100-epoch mark (red arrow). Mini-batch related noise obscures the exact cross-over location.}
\label{fig:nn_loss_plot}
\end{figure}

Tuning was conducted in the order shown in Figure \ref{fig:nn_tuning_plots}. There are distinct maxima for the CV-averaged WDS4 AUC values in each hyperparameter case. The final selected values include: learning rate of 0.01, $\lambda$ value of $2\times10^{-4}$, batch size of 100, and drop rate of 0.1. Minima in the average loss calculation align with these values for all except $\lambda$, the regularization parameter on model weights. This matches the expected behavior described in equation \ref{eq:nn_cost_function}, where $\lambda$ controls the balance between loss and sum of squared weights in the cost function. As Figure \ref{fig:nn_tuning_plots}B shows, the loss term is minimized as $\lambda$ approaches zero.

Figure \ref{fig:nn_loss_plot} illustrates the WDS4 training progress as a time series of loss and AUC. The variance in both plots reflects the noise introduced by mini-batch training, but convergence takes place quickly. Increasing deviation of the validation loss and, to a lesser extent, AUC values at greater epoch counts indicates the model is overfitting the training data. The cross-over of the training and validation lines, which corresponds with the tuned number of epochs for the final model, occurs somewhere close to 100 epochs for WDS4.

\subsubsection{Optimized Model Results}

The NN model was re-trained for each of the three data sets using the hyperparameter values listed in Table \ref{tab:nn_tuning}. Data sparsity is particularly problematic for the WDS model; even after careful tuning, the out-of-sample accuracy does not exceed 75\%, and test set AUC is $\approx 86\%$. Data augmentation in WDS4 and WDS8 has a strong pay-off in the results. Models trained on both data sets exhibit $\approx95\%$ accuracy and AUC values $>99\%$ (Table \ref{tab:nn_tuning}).

Figure XX shows the confusion matrix for WDS4.

\begin{figure}[!htp]
\centering
\includegraphics[width=.6\textwidth]{templates/images/Figure-NN-ConfusionMatrix.png}
